...
public class DwsTradeSkuOrderWindowAsyncCache extends BaseApp {
    public static void main(String[] args) {
        new DwsTradeSkuOrderWindowAsyncCache().start(10029, 4, "dws_trade_sku_order_window_sync_cache", Constant.TOPIC_DWD_TRADE_ORDER_DETAIL);
    }
...
//        reduceBeanStream.print();
        ...
        // 异步IO关联sku_info
        SingleOutputStreamOperator<TradeSkuOrderBean> fullDimStream = joinDim(reduceBeanStream);
...
//                .sinkTo(FlinkSinkUtil.getDorisSink(Constant.DWS_TRADE_SKU_ORDER_WINDOW));
    ...
    private SingleOutputStreamOperator<TradeSkuOrderBean> joinDim(SingleOutputStreamOperator<TradeSkuOrderBean> reduceBeanStream) {
        SingleOutputStreamOperator<TradeSkuOrderBean> skuInfoStream = AsyncDataStream.unorderedWait(reduceBeanStream, new DimAsyncFunction<TradeSkuOrderBean>() {
            ...
            public void join(TradeSkuOrderBean tradeSkuOrderBean, JSONObject dim) {
                ...
                tradeSkuOrderBean.setSpuId(dim.getString("spu_id"));
                tradeSkuOrderBean.setSkuName(dim.getString("sku_name"));
            }
        }, 60, TimeUnit.SECONDS);

        SingleOutputStreamOperator<TradeSkuOrderBean> spuInfoStream = AsyncDataStream.unorderedWait(reduceBeanStream, new DimAsyncFunction<TradeSkuOrderBean>() {
            @Override
            public String getId(TradeSkuOrderBean tradeSkuOrderBean) {
